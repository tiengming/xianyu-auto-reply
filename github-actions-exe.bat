@echo off
REM === XianyuAutoReply EXE Build Script (CI / GitHub Actions version) ===
REM 本脚本基于本地构建脚本改造，供 GitHub Actions Windows Runner 非交互执行。
REM 主要变更见文末说明，核心原则：不阻塞（无 choice / pause）、失败即时退出（exit /b 1）。

chcp 65001 >nul
cd /d "%~dp0"

echo ============================================
echo   XianyuAutoReply - CI Build
echo ============================================
echo.

REM --- SKIP_FRONTEND 通过环境变量传入（workflow_dispatch 输入或 env 块设置） ---
REM     0 = 重新构建前端（默认），1 = 跳过前端构建
if not defined SKIP_FRONTEND set SKIP_FRONTEND=0
echo [INFO] SKIP_FRONTEND=%SKIP_FRONTEND%
echo.
...
REM 导出给后续 workflow 步骤（artifact 命名 / release 使用）
...
REM --- Do not trust errorlevel alone here. If the process is interrupted mid-compile
REM     (e.g. CI job timeout sends a break signal), cmd.exe's batch interpreter can
REM     prompt "Terminate batch job (Y/N)?" and, with non-interactive stdin on a CI
REM     runner, silently resolve to "continue" without correctly propagating a
REM     failure errorlevel. Explicitly verify the expected output binary exists.
if not exist "build_output\main.dist\XianyuAutoReply.exe" (
    echo [ERROR] build_output\main.dist\XianyuAutoReply.exe not found after Nuitka step.
    echo         Compile was likely interrupted ^(CI timeout / cancellation^) rather than genuinely completed.
    goto :fail
)
...
REM --- Install/reuse Chromium in a persistent cache dir outside build_output/release
REM     (both are wiped at the start of every run), so a CI-level cache action can
REM     keep this directory across runs and avoid re-downloading ~300MB each build.
if not defined PLAYWRIGHT_CACHE_DIR set "PLAYWRIGHT_CACHE_DIR=%~dp0.playwright-cache"
mkdir "%PLAYWRIGHT_CACHE_DIR%" 2>nul
set "PLAYWRIGHT_BROWSERS_PATH=%PLAYWRIGHT_CACHE_DIR%"
echo [INFO] Installing/verifying Chromium in cache dir: %PLAYWRIGHT_CACHE_DIR%
...
echo [INFO] Copying Chromium from cache into package dir...
xcopy /E /I /Y /Q "%PLAYWRIGHT_CACHE_DIR%" "%PACKAGED_BROWSER_DIR%" >nul
if errorlevel 1 (
    echo [ERROR] Failed to copy Chromium into package dir.
    goto :fail
)
...
REM 导出给后续 workflow 步骤（artifact 路径确认 / release 附件命名）
if defined GITHUB_ENV (
...
echo.
echo ============================================
echo   Build FAILED. See log above for details.
echo ============================================
exit /b 1
